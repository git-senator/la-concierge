...
(function () {
  ...
  (function () {
    ...
    window.addEventListener('wheel', function (e) {
      ...
      /* Перескоков между разделами здесь больше нет.

         Было так: раздел считался дочитанным, когда его нижняя граница
         входила в экран, и колесо переносило к следующему. Но после
         переноса заголовок следующего раздела встаёт под шапкой, а
         нижняя граница предыдущего оказывается на восемьдесят пикселей
         ниже верхнего края экрана — то есть условие снова верно. Любое
         движение колеса вниз возвращало на то же место, и страница
         запиралась: замер показал качели между 3489 и 3568 пикселями у
         «Направлений», из которых нет выхода ни вниз, ни вверх — на
         девятьсот миллисекунд после каждого прыжка колесо ещё и молчит.

         Первый экран остаётся как был: там прыжок в одну сторону и
         возврата в ту же точку не бывает.                          */
    }, { passive: false });
  })();
...
(function () {
  ...
  function snap() {
    ...
    if (hero && имя && ряд && сетка && столб) {
      ...
      /* Слово MONTERO владелец попросил опустить на два десятка
         точек, не тронув ничего под ним. Двигаем только строку и
         только свойством translate: разметка не меняется, подпись
         под знаком и кубики остаются на своих местах. Расчёт поля
         сверху спуска не знает: он ведётся по коробке строки, а она
         на месте — оттого ниже ничего и не двигается. */
      var ВНИЗ = 20;
      имя.style.translate = '0 ' + ВНИЗ + 'px';
      ...
      if (тексты.length) {
        /* Потолок — 16 точек: столько набрано в карточке услуги,
           владелец показал на неё как на образец. Сначала пробуем
           его целиком, и только если текст не влез — делим пополам. */
        var ОБРАЗЕЦ = 16;
        тексты.forEach(function (эл) { эл.style.fontSize = ОБРАЗЕЦ + 'px'; });
  ...
  /* Пока экран под заслонкой, пересчёт идёт по будильникам — они
     ловят запоздавшие начертания. Как только экран показан, будильники
     гасим: после показа страница не должна шевелиться вовсе. */
  var будильники = [400, 1200, 2500].map(function (ms) {
    return setTimeout(snap, ms);
  });
  var открыть = function () {
    будильники.forEach(clearTimeout);
...
(function () {
  ...
  function place() {
    ...
    /* Иностранные языки набраны Cormorant Garamond, и его буквенный
       ящик глубже русского: та же формула опускала линию на десять
       точек ниже, под «WHAT WE DELIVER» до строки под ним оставалось
       девять точек против девятнадцати в русском — линия почти
       садилась на текст. Владелец попросил приподнять её на половину
       этого просвета. Гарнитура у всех четырёх иностранных одна,
       поэтому и подъём один. Русский не трогаем: там линия стоит так,
       как он её принял.                                            */
...
(function () {
  ...
  /* Смена языка пересобирает форму: старый узел удаляется целиком
     (RequestForm.relang), а на его месте вырастает новый. Всё, что мы
     в него перенесли, из документа при этом исчезает.

     Замер после переключения языка на компьютере: три кнопки связи
     пропадали со страницы совсем — они лежали внутри формы, — а
     подсказки в полях становились пустыми: их текст снят с подписей и
     на новых узлах его нет. Владелец увидел вторую половину: поля
     стояли без надписей, пока страницу не перезагрузишь.

     Поэтому после каждой пересборки возвращаем кнопки в новую форму и
     заново ставим подсказки. Второй заход через такт — на случай,
     если пересборка подпишется на это же событие позже нас.

     Ниже 981 ничего не делаем: там форма стоит на своём месте, кнопки
     тоже, а подписи видны и подсказки не нужны.                     */
  function afterRelang() {
    if (!moved) return;
    var rf = mount.querySelector('.rf');
    if (rf && dir && dir.parentNode !== rf) rf.appendChild(dir);
    hints(true);
  }
  ...
  window.addEventListener('load', place);
  document.addEventListener('langchange', function () {
    place();                       /* другой язык — другая длина строк */
    afterRelang();
    setTimeout(afterRelang, 0);
  });
})();
